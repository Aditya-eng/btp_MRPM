% NOTE: paper.pdf in this folder was compiled BEFORE the citation-order and
% per-equation-citation edits below (no LaTeX toolchain was available to
% recompile at edit time). Recompile this .tex (e.g. via Overleaf) to
% regenerate a matching PDF before using/submitting it.
...
% }
\author{
\IEEEauthorblockN{Aditya Garg, Ashu Anand, Gargi Singh, Aditya Mohan and Jyoti Yadav}
\IEEEauthorblockA{\textit{Dept.\ of Instrumentation and Control Engineering} \\
\textit{Netaji Subhas University of Technology (NSUT)} \\
Delhi, India \\
Email: \{aditya.garg.ug23, ashu.anand.ug23, gargi.singh.ug23, aditya.mohan.ug23, jyoti.yadav\}@nsut.ac.in}
}
...
TN, Deepika Sree, and S. Aditya~\cite{base} proposed a Modified Russian
...
\cite{bansal,gokhale} exploit parallel sub-product formation and
...
TN, Deepika Sree, and S. Aditya~\cite{base} combine a modified RPM with a
...
high-frequency noise while retaining the diagnostic band, consistent
with the power- and area-constrained biomedical acquisition context of
\cite{lotfabadi,ajay}. The filter is
an 8-tap linear-phase FIR designed by the windowed-sinc method with a
Hamming window, following the RPM-based linear-phase FIR design practice
of \cite{gunasekaran}. Linear phase is not merely convenient: a non-linear
...
$b_{-1} \equiv 0$, following the modified-Booth recoding convention of
\cite{prabhu}. The control signals are
...
\noindent The recoding rules \eqref{eq:one}, \eqref{eq:two}
and \eqref{eq:neg} produce a signed digit in $\{-2,-1,0,+1,+2\}$ at each
step, as in the standard Booth partial-product formation of \cite{prabhu}.
The signals
$\mathrm{one}_k$ and $\mathrm{two}_k$ are mutually exclusive by
construction, so the partial-product magnitude is formed by selection
...
negation and alignment, extending the base MRPM datapath of \cite{base}
with radix-4 recoding:
...
following the Han-Carlson adder formulation of \cite{rao}.
Pre-processing forms the bitwise generate and propagate terms of
\eqref{eq:gp},
...
\noindent A logarithmic-depth prefix network, of the kind analysed for
Kogge-Stone-family adders in \cite{penchalaiah,raju}, then combines
these terms; the carry into bit $i+1$ and the sum bit follow as
\eqref{eq:sum},
...
\noindent in which $(G_i, P_i)$ denote the group generate and propagate
...
output, as realised in the RPM-based FIR of \cite{gunasekaran},
...
image, a folding identity used for symmetric FIR realisation in
\cite{kumar,gunasekaran}:
...
Bit-growth is bounded as follows, using the standard fixed-point
accumulator sizing analysis for FPGA multiply-accumulate datapaths of
\cite{paz}. Each mirror-pair pre-add of two
...
the device carried by the Sipeed Tang Nano 9K board~\cite{tangnano} and a substantially
...
not exist in the direct form. The realised transformation, an
instance of the multiplier-for-adder trade central to RPM-based folded
FIR designs such as \cite{gunasekaran}, is thus given
by \eqref{eq:foldcost},
...
\begin{tabular}{@{}lcc@{}}
\toprule
 & \textbf{Base \cite{base}} & \textbf{Proposed work} \\
\midrule
...
\bibitem{sharma}
Sharma, Mahendra, and Dr SK Singh. "New technique based Peasant multiplication for effcient signal processing applications." Indonesian Journal of Electrical Engineering and Computer Science 8, no. 3 (2017): 726-729.

\bibitem{lotfabadi}
Lotfabadi, Shahin Sanayei, Andy Ye, and Sridhar Krishnan. "Measuring the power efficiency of subthreshold FPGAs for implementing portable biomedical applications." Microprocessors and Microsystems 36, no. 3 (2012): 151-158.

\bibitem{ajay}
Ajay, Arathi, and R. Mary Lourde. "VLSI Implementation of an improved multiplier for FFT Computation in Biomedical Applications." In 2015 IEEE Computer Society Annual Symposium on VLSI, pp. 68-73. IEEE, 2015.

\bibitem{phuse}
Phuse, Ashish S., and Pankaj P. Tasgaonkar. "Design and implementation of different multiplier techniques and efficient MAC unit on FPGA." In 2022 International Conference on Signal and Information Processing (IConSIP), pp. 1-5. IEEE, 2022.
...
\bibitem{gokhale}
Gokhale, G. R., and P. D. Bahirgonde. "Design of Vedic-multiplier using area-efficient Carry Select Adder." In 2015 International conference on Advances in computing, Communications and Informatics (ICACCI), pp. 576-581. IEEE, 2015.

\bibitem{bansal}
Bansal, Yogita, Charu Madhu, and Pardeep Kaur. "High speed vedic multiplier designs-A review." 2014 Recent Advances in Engineering and Computational Sciences (RAECS) (2014): 1-6.

\bibitem{khurshid}
Khurshid, Burhan. "FPGA‐Based Resource‐Optimal Approximate Multiplier for Error‐Resilient Applications." International Journal of Circuit Theory and Applications 53, no. 9 (2025): 5518-5537.

\bibitem{approx2025}
Arunkumar, K., S. Sushma, and Vangala Siva Teja. "Improving Error Resilience in FPGA-Based Multi-Level Approximate Multipliers Using Modified Full Adders." In 2025 3rd International Conference on Device Intelligence, Computing and Communication Technologies (DICCT), pp. 153-158. IEEE, 2025.

\bibitem{base}
TN, Deepika Sree, and S. Aditya. "A Low cost area-efficient modified Russian peasant multiplier (MRPM) for biomedical applications." Integration (2025): 102474.

\bibitem{abed}
Abed, Sa'ed, Bassam Jamil Mohd, Zaid Al‐bayati, and Sahel Alouneh. "Low power Wallace multiplier design based on wide counters." International Journal of Circuit Theory and Applications 40, no. 11 (2012): 1175-1185.

\bibitem{badawi}
Badawi, Aiman, Ali Alqarni, Abdullah Aljuffri, Mohammed S. BenSaleh, Abdulfattah M. Obeid, and Syed Manzoor Qasim. "FPGA realization and performance evaluation of fixed-width modified Baugh-Wooley multiplier." In 2015 Third International Conference on Technological Advances in Electrical, Electronics and Computer Engineering (TAEECE), pp. 155-158. IEEE, 2015.

\bibitem{paz}
Paz, Pedro, and Mario Garrido. "Efficient implementation of complex multipliers on FPGAs using DSP slices." Journal of Signal Processing Systems 95, no. 4 (2023): 543-550.

\bibitem{kumar}
Kumar, C. Uthaya, and B. Justus Rabi. "Design and implementation of modified russian peasant multiplier using msqrtcsla based fir filter." Indian Journal of Science and Technology 9, no. 7 (2016): 1-6.

\bibitem{gunasekaran}
Gunasekaran, K., and M. Manikandan. "High speed reconfigurable FIR filter using Russian peasant multiplier with Sklansky adder." Research Journal of Applied Sciences, Engineering and Technology 8, no. 24 (2014): 2451-2456.

\bibitem{rao}
Rao, E. Jagadeeswara, T. Ramanjaneyulu, and K. Jayaram Kumar. "Advanced multiplier design and implementation using Han-Carlson adder." In 2018 International Conference on Intelligent and Innovative Computing Applications (ICONIC), pp. 1-5. IEEE, 2018.

\bibitem{penchalaiah}
Penchalaiah, U., and Siva Kumar Vg. "Design of high-speed and energy-efficient parallel prefix kogge stone adder." In 2018 IEEE International Conference on System, Computation, Automation and Networking (ICSCA), pp. 1-7. IEEE, 2018.

\bibitem{raju}
Raju, Aradhanan, and Sudhir Kumar Sa. "Design and performance analysis of multipliers using Kogge Stone Adder." In 2017 3rd International Conference on Applied and Theoretical Computing and Communication Technology (iCATccT), pp. 94-99. IEEE, 2017.

\bibitem{anjana}
Anjana, R., B. Abishna, M. S. Harshitha, E. Abhishek, V. Ravichandra, and M. S. Suma. "Implementation of vedic multiplier using Kogge-stone adder." In 2014 International Conference on Embedded Systems (ICES), pp. 28-31. IEEE, 2014.
