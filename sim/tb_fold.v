    ...
    initial begin
        ...
        for(i=0;i+1<nsamp;i=i+1)
            if(yq[i+1]!==$signed(ya[i])) begin errors=errors+1;
                if(errors<6) $display("FOLD MISMATCH #%0d got=%0d exp=%0d",i,yq[i+1],$signed(ya[i])); end
        $display("FOLDED FIR self-check: %0d mismatches over %0d samples (0=perfect)",errors,nsamp-1);
        $finish;
    end
endmodule
